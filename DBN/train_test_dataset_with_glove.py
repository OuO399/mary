...
def save_without_mutation_file(project_name,version,train_X_dict,train_Y_dict,test_X_dict,test_Y_dict):
    save_train_set_tokens_in_txt(project_name,version,train_X_dict)

def save_mutation_file_used_in_oversample(project_name,version,train_X_dict,train_Y_dict,test_X_dict,test_Y_dict):
    ...
    for i in numlist:
        ...
        token_list = train_tokens_dict_with_mutation[key]
        num_tokens[key] = token_list
        add_tokens[key] = token_list
        train_Y_dict[key] = 1
    save_train_set_tokens_in_txt(project_name,version,add_tokens)
    with open("../numtokens_with_GloVe/{}_{}_with_mutation_file.pkl".format(project_name,version),"wb") as f:
        pickle.dump(num_tokens,f)
    return train_Y_dict

def save_manual_mutation_file_used_in_oversample(project_name,version,train_X_dict,train_Y_dict,test_X_dict,test_Y_dict):
    ...
    for i in numlist:
        ...
        train_Y_dict[key] = 1
    save_train_set_tokens_in_txt(project_name,version,add_tokens)
    with open("../numtokens_with_GloVe/{}_{}_with_manual_mutation_file.pkl".format(project_name,version),"wb") as f:
        pickle.dump(num_tokens,f)
    return train_Y_dict

def save_μbert_mutation_file_used_in_oversample(project_name,version,train_X_dict,train_Y_dict,test_X_dict,test_Y_dict):
    ...
    for i in numlist:
        ...
        train_Y_dict[key] = 1
    save_train_set_tokens_in_txt(project_name,version,add_tokens)
    with open("../numtokens_with_GloVe/{}_{}_with_μbert_mutation_file.pkl".format(project_name,version),"wb") as f:
        pickle.dump(num_tokens,f)
    return train_Y_dict
...
def save_for_glove(projects,cut_length):  
    ...
    for project in projects_with_version.keys():
        ...
        for version in versions:
            ...
            save_without_mutation_file(project,version,train_X,train_Y,test_X,test_Y)
            train_Y_with_mutation = save_mutation_file_used_in_oversample(project,version,train_X,train_Y,test_X,test_Y)
            train_Y_with_manual_mutation = save_manual_mutation_file_used_in_oversample(project,version,train_X,train_Y,test_X,test_Y)
            train_Y_with_μbert_mutation = save_μbert_mutation_file_used_in_oversample(project,version,train_X,train_Y,test_X,test_Y)

            with open("./train_data_with_glove/{}_{}_train_X_{}.pkl".format(project,version,cut_length),"wb") as f :
                pickle.dump(train_X,f)

            with open("./train_data_with_glove/{}_{}_without_mutation_train_Y_{}.pkl".format(project,version,cut_length),"wb") as f :
            ...
            with open("./train_data_with_glove/{}_{}_with_manual_mutation_train_Y_{}.pkl".format(project,version,cut_length),"wb") as f :
                pickle.dump(train_Y_with_manual_mutation,f)

            with open("./train_data_with_glove/{}_{}_with_μbert_mutation_train_Y_{}.pkl".format(project,version,cut_length),"wb") as f :
                pickle.dump(train_Y_with_μbert_mutation,f)

            with open("./test_data_with_glove/{}_{}_test_X_{}.pkl".format(project,version,cut_length),"wb") as f :
                pickle.dump(test_X,f)

            with open("./test_data_with_glove/{}_{}_test_Y_{}.pkl".format(project,version,cut_length),"wb") as f :
                pickle.dump(test_Y,f)

    #         train_X_dict['{}_{}'.format(project,version)] = train_X
...
if __name__ == '__main__':
    ...
    # projects_with_version = {"jEdit":['4.3'],"ivy":['2.0']}
    # projects = ['ivy']
    # projects = ['ant']
