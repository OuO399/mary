...
def manual_mutation_oversample(project_name,version,cut_length,train_X_dict,train_Y_dict,test_X_dict,test_Y_dict):
    ...
    print("token_list_length:{}".format(len(train_tokens_dict_with_mutation)))
    if (len(train_tokens_dict_with_mutation)<=num) :
        numlist = [i for i in range(len(train_tokens_dict_with_mutation))]
    else:
        numlist = random.sample(range(0,len(train_tokens_dict_with_mutation)-1),num)
...
def dbn_train(project,version,data_type,cut_length,train_X,train_Y,test_X,test_Y):
    ...
    print('{}_{}_{}_{} within DBN train started'.format(project,version,data_type,cut_length))
    within_dbn.fit(within_dbn_input)

    print('{}_{}_{}_{} within DBN train ended'.format(project,version,data_type,cut_length))
    within_train_dbn_output = within_dbn.transform(within_dbn_input)
    print(within_train_dbn_output.shape)

    np.save('./train_data/{}_{}_train_X_{}_{}.npy'.format(project,version,data_type,cut_length), within_train_dbn_output)

    np.save('./train_data/{}_{}_train_Y_{}_{}.npy'.format(project,version,data_type,cut_length), np.array(train_Y))

    within_test_dbn_output = within_dbn.transform(test_X)
    np.save('./test_data/{}_{}_test_X_{}_{}.npy'.format(project,version,data_type,cut_length), within_test_dbn_output)

    np.save('./test_data/{}_{}_test_Y_{}_{}.npy'.format(project,version,data_type,cut_length), np.array(test_Y))
...
def handle_batch(projects,cut_length):
    ...
    for project in projects:
        ...
        for version in versions:
            ...
            without_oversample(project,version,cut_length_string,train_X,train_Y,test_X,test_Y)
            mutation_oversample(project,version,cut_length_string,train_X,train_Y,test_X,test_Y)
            ROS_oversample(project,version,cut_length_string,train_X,train_Y,test_X,test_Y)
            manual_mutation_oversample(project,version,cut_length_string,train_X,train_Y,test_X,test_Y)
    ...
    #         #所有项目一起跑
    #         Processes.append(Process(target=without_oversample,args=(project,version,cut_length_string,train_X,train_Y,test_X,test_Y)))
    #         Processes.append(Process(target=mutation_oversample,args=(project,version,cut_length_string,train_X,train_Y,test_X,test_Y)))
    #         Processes.append(Process(target=ROS_oversample,args=(project,version,cut_length_string,train_X,train_Y,test_X,test_Y)))
    #         Processes.append(Process(target=manual_mutation_oversample,args=(project,version,cut_length_string,train_X,train_Y,test_X,test_Y)))
    # start_time = time.time()
