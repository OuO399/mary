...
public class MaskedTokenMutants {
    ...
    public String getFullMethodStr(CtMethod method){
        int method_start = ((CtMethodImpl) method).getPosition().getSourceStart();
        int method_end = method.getPosition().getSourceEnd();
        String fullMethodStr = originalClass.substring(method_start,method_end+1);
        fullMethodStr = fullMethodStr.replaceFirst("^\\s*", "");
        fullMethodStr = fullMethodStr.replaceFirst("\\s++$", "");
        // System.out.println(fullMethodStr);
