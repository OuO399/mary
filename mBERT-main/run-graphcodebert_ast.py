...
# url = f"http://127.0.0.1:8080/graphc_use_ast{PROCESS_LABEL}"
...
import javalang
from transformers import RobertaConfig, RobertaTokenizer, RobertaForMaskedLM, pipeline,RobertaModel,AutoTokenizer,AutoModelForMaskedLM
# from load_model import fill_mask_graphc_cpu,model_graphc_cpu,tokenizer_graphc_cpu
...
tokenizer = RobertaTokenizer.from_pretrained(GRAPH_CODEBERT_MLM)
...
# tokenized_CODE = tokenizer_graphc_cpu.tokenize(CODE)
tokenized_CODE = tokenizer.tokenize(CODE)
# print(tokenized_CODE)
tokens_count = str(len(tokenized_CODE))

mask_index = tokenized_CODE.index("<mask>")
start_index = max(0, mask_index - 128)
stop_index = min(len(tokenized_CODE), mask_index + 128) - 1
SHRINKED_TOKENS = tokenized_CODE[start_index:max(stop_index,256)]
#assert(len(SHRINKED_TOKENS)) #512 is the maximum sequence length for codebert model
# tokens_ids = tokenizer_graphc_cpu.convert_tokens_to_ids(SHRINKED_TOKENS)
# SHRINKED_CODE = tokenizer_graphc_cpu.decode(tokens_ids)
